...
export function ActivityCard({ action, isSelected, onClick }: ActivityCardProps) {
  ...
  return (
    <button
      ...
      className={`relative w-full text-left pl-6 pr-4 py-4 rounded-lg border transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 dark:focus:ring-slate-500 dark:focus:ring-offset-[#050505] ${
        isSelected
          ? `bg-white dark:bg-[#0F0F10] border-slate-300 dark:border-[#232326]`
          : `bg-white dark:bg-[#0F0F10] border-slate-200 dark:border-[#232326] hover:bg-slate-50 dark:hover:bg-[#161618]`
      }`}
    >
      {/* left colored strip */}
      <span className={`absolute left-0 top-0 bottom-0 w-1.5 rounded-l-md ${config.strip}`} />
